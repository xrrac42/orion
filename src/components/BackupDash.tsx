...
// Use the path-based dashboard endpoint: /api/dashboard/{analysis_id}
async function getDashboardData(analysisId: string, month?: number | null, year?: number | null) {
  if (!analysisId) throw new Error('ID da análise é inválido');
  ...
  const url = `${API_BASE_URL}/api/dashboard/${analysisId}${qs ? `?${qs}` : ''}`;
  ...
  useEffect(() => {
    async function fetchDashboardDetails() {
      ...
      try {
        ...
        const analysisToUse = activeAnalysisId || selectedBalanceteId;
        if (!analysisToUse) {
          console.debug('[dashboard] no analysis id to fetch');
          return;
        }

        const dashboardData = await getDashboardData(analysisToUse, filterMonth, filterYear);
        console.log('[dashboard] fetchDetails: dashboardData', dashboardData);
