...
CREATE TABLE public.clients (
    ...
    nome TEXT NOT NULL,
DROP TABLE IF EXISTS public.monthly_analyses CASCADE;
    cpf TEXT,
    email TEXT,
DROP TABLE IF EXISTS public.monthly_analyses CASCADE;
    telefone TEXT,
...
DROP TABLE IF EXISTS public.monthly_analyses CASCADE;
    ...
    client_name TEXT NOT NULL, -- Nome do cliente extraído pela IA
    status TEXT NOT NULL DEFAULT 'processing' CHECK (status IN ('processing', 'completed', 'failed')),
    source_file_path TEXT NOT NULL, -- Caminho para o arquivo PDF original no Storage
    source_file_name TEXT NOT NULL, -- Nome original do arquivo
...
-- =======================================================
-- 4. TABELA DE ENTRADAS FINANCEIRAS (REFATORADA)
-- =======================================================

CREATE TABLE public.financial_entries (
DROP TABLE IF EXISTS public.financial_entries CASCADE;
    id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    analysis_id BIGINT REFERENCES public.monthly_analyses(id) ON DELETE CASCADE,
    client_id UUID REFERENCES public.clients(id) ON DELETE CASCADE,
...
DROP TABLE IF EXISTS public.quarantine_files CASCADE;
    original_data JSONB,
    created_by UUID REFERENCES auth.users(id),
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE public.file_uploads (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    client_id UUID REFERENCES public.clients(id) ON DELETE CASCADE,
    analysis_id BIGINT REFERENCES public.monthly_analyses(id) ON DELETE SET NULL,
    file_name TEXT NOT NULL,
    file_path TEXT NOT NULL,
DROP TABLE IF EXISTS public.balancetes CASCADE;
    mime_type TEXT,
    status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'pre_check', 'confirmed', 'processing', 'completed', 'failed', 'quarantine')),
    pre_check_result JSONB, -- Resultado da pré-análise (cliente, data, etc.)
    ...
    DROP TABLE IF EXISTS public.file_uploads CASCADE;
-- =======================================================
-- 6. TABELA DE QUARENTENA (ATUALIZADA)
DROP TABLE IF EXISTS public.quarantine_files CASCADE;
-- =======================================================
...
-- NOTA: Esta tabela será preenchida automaticamente a partir das monthly_analyses
...
-- Índices para monthly_analyses
CREATE INDEX idx_monthly_analyses_client_date ON public.monthly_analyses(client_id, report_date);
CREATE INDEX idx_monthly_analyses_status ON public.monthly_analyses(status);
CREATE INDEX idx_monthly_analyses_month_year ON public.monthly_analyses(reference_year, reference_month);
CREATE INDEX idx_monthly_analyses_created_at ON public.monthly_analyses(created_at);

-- Índices para financial_entries (atualizados)
CREATE INDEX idx_financial_entries_analysis ON public.financial_entries(analysis_id);
...
-- Índices para file_uploads (atualizados)
CREATE INDEX idx_file_uploads_client ON public.file_uploads(client_id);
CREATE INDEX idx_file_uploads_analysis ON public.file_uploads(analysis_id);
...
-- Índices para balancetes (atualizados)
CREATE INDEX idx_balancetes_analysis ON public.balancetes(analysis_id);
...
-- Habilitar RLS em todas as tabelas (atualizado)
...
-- Políticas para monthly_analyses
CREATE POLICY "Ver análises de clientes próprios" ON public.monthly_analyses
    FOR SELECT USING (
        EXISTS (
...
CREATE POLICY "Criar análises para clientes próprios" ON public.monthly_analyses
...
CREATE POLICY "Atualizar análises próprias" ON public.monthly_analyses
    FOR UPDATE USING (
        created_by = auth.uid() OR
        EXISTS (
            SELECT 1 FROM public.user_profiles 
...
-- Políticas para financial_entries (atualizadas)
CREATE POLICY "Ver entradas de análises próprias" ON public.financial_entries
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.monthly_analyses ma
            JOIN public.clients c ON ma.client_id = c.id 
            WHERE ma.id = analysis_id AND (
...
CREATE POLICY "Inserir entradas para análises próprias" ON public.financial_entries
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.monthly_analyses ma
            JOIN public.clients c ON ma.client_id = c.id 
            WHERE ma.id = analysis_id AND c.created_by = auth.uid()
        )
    );
